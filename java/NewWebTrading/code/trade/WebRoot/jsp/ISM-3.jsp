...
<form action="../EnterShk.do?CLV=${sessionScope.CLV}" name="EnterSHK" id="EnterSHK" method="post">
    <div class="page-content">
        <div class="form-table-center ui-corner-all">
            <div class="title">
              <bean:message key="Product_SHK"/></div>
            <table>
                <tr>
                    <td class="message">
                     <bean:message key="PurchSHKY"/> 
                    </td>
                </tr>
                  
                <tr>
             		 <td>
             		 	<input type="hidden" id="productId" name="productId" value="SHK"/>
             		 	<input type="hidden" id="CLV" name="CLV" value="WT25"/>
             		 	<input type="hidden" id="loginId" name="loginId" value="${User.tradeInfoObject.custCode}"/>
                        <input type="submit" class="yellow-btn-long" value="<bean:message key="EnterSHK"/>" />
                        <input type="button" class="prospectus-btn" value="<bean:message key="lable.quote.cancelquote"/>" onclick="javascript:location.href='${pageContext.request.contextPath}/jsp/ISM-Cancel.jsp'" />
                    </td>
                </tr>
            </table>
        </div>
