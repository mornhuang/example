    ...
    <div class="page-content">
        <table class="form-table ui-corner-all">
            ...
            <tr class="alternating">
                ...
                <td>
                    <strong><bean:message key="${resultPrgStatus}"/></strong>
                </td>
            </tr>
            <c:forEach var="resultStr" items="${resultList }" varStatus="status">
            	<c:choose>
            		<c:when test="${status.count%2==0}">
            			<tr class="alternating">
			                <th>
			                	<c:if test="${status.index==0}">
			                    	<bean:message key="message.ipo.status.row18"/>
			                    </c:if>
			                </th>
			                <td>
			                    <strong><bean:message key="${resultStr}"/></strong>
			                </td>
		            	</tr>
            		</c:when>
            		<c:otherwise>
