...
// isPositiveFloat (s)                 True if string s is an Float > 0.
...
function isPositiveAnyway(s) {
    ...
    var i = s.indexOf('.');
    if (i > -1) {
        var fc = s.substring(0, i);
        var bc = s.substring(i + 1, s.length);
        return (isPositiveInteger(fc) || isPositiveInteger(trimLeadingZero(bc)));
    }
    else {
...
// steve added
// this function get number of decimal point
// arg0 = src, arg1 = can be empty {true|false}
// -1 if it is not a float or integer,
// 0 if there is no decimal place,
// x [1-n] represents the number of decimal places
function checkDecimalPoint(s) {
    if (isEmpty(s)) {
        if (isFloat.arguments.length == 1)
            return defaultEmptyOK ? 0 : -1;
        else
            return (checkDecimalPoint.arguments[1] == true ? 0 : -1);
    }

    if (isSignedFloat(s)) {
        return s.indexOf('.') != -1 ? (s.substring(s.indexOf('.') + 1, s.length)).length : 0;
    }
    else {
        return -1;
    }
