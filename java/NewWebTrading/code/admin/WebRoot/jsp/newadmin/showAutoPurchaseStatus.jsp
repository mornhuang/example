...
<table border=0 width=95% cellspacing=0 cellpadding=2  align="left">
	<tr>
		...
		<table width="100%" border="0">
			<tr>
				<td><strong>Auto Purchase Progress</strong></td>
				<td align="right">
		...
		<c:choose>
			<c:when test="${isReseted==true}">
				<tr>
					<td>The latest process result has been cleared.</td>
				</tr>
				<tr>
					<td align="center"><br>
				...
				<tr>
					...
					<table width="85%" border="0">
						...
						<tr>
							...
							<td>
								<c:if test="${lackMoney == 0}">${lackMoney}</c:if>
								<c:if test="${lackMoney ne 0}"><font color="red">${lackMoney}</font></c:if>	
							</td>
						</tr>
						<tr>
					...
					<c:when test="${isProcessing==true}">
						<tr>
							...
							<input type="button" name="Refresh"
								onclick="window.location.href = '<%=request.getContextPath()%>/showAutoPurchaseStatus.do?method=showAutoPurchaseStatus';"
					...
					<c:otherwise>
						<tr>
							...
							<input type="button" name="Release"
								onclick="window.location.href='<%=request.getContextPath()%>/showAutoPurchaseStatus.do?method=showAutoPurchaseStatus&action=release';"
				...
				<logic:present name="autoPurchaseErrorList">
					<tr>
						...
						<table width="85%" border="0">
							...
							<tr bgcolor="#A5B7C5">
								<th width="35%">AutoPurchase ID</th>
