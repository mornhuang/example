...
function checkQty(qty, stockCode) {
	...
	if (!/\d+/.test(qtyVal)) {
		...
		if (lotsize == 0 || qtyVal <= 0 || (lotsize && qtyVal % lotsize != 0) ) {
			alert($.sht.msg["sys018"]);
			$(qty).select()[0].focus();
			return false;
		}else if(qtyVal > 3000 * lotsize){
			alert($.sht.msg["sys023"]);
			$(qty).select()[0].focus();
			return false;
		}
...
function checkModfiyQty(newQty, initQty, filledQty, changedQty) {
	...
	if (newQtyVal < filledQty || newQtyVal > initQty + changedQty) {
		alert($.sht.msg["SHT003"]);
		return false;
	}
	return true;
...
function checkedNochanged(qty,newqty,price1,price2){
	if($(qty).val()==$(newqty).val()&&$(price1).val()==$(price2).val()){
		alert($.sht.msg["sys024"]);
		return false;
	}
	return true;
...
function checkTriggerPrice(triggerPrice, orderPrice, orderSide, conditionType) {
	...
	if (!/^\d+\.?\d{0,3}$/.test(triggerPriceVal)) {
...
function fillLabels(form, data) {
	var obj,txt;
	for (var prop in data) {
		obj = $(form).find("#l_" + prop);
		txt = obj.text();
		obj.text(txt.replace(/\(\w+\)/g, '(' + data['currency'] + ')'));
	}
	obj=null;txt=null;prop=null;
}
