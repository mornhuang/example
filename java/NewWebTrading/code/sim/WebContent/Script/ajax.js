function checkHeader(response) {
	if (response.returnCode == "NORMAL") {
		return true;
	} else if (response.returnCode == "WARN") {
		...
		alert(response.errorCode && $.sht.msg[response.errorCode] ? $.sht.msg[response.errorCode] : response.errorCode);
...
function postCallMos(request, fn) {
	postData(request, "../webCallMos.do", fn);
}
