...
#define CONFIG_MY_LOGO
#define CONFIG_MINI_LOGO

#if defined(CONFIG_MINI_LOGO) && !defined(CONFIG_MY_LOGO)
...
/* HACKED by zerow_jp*/
#ifdef CONFIG_MINI_LOGO
/* mini logo (for SMP)*/
/* define
	MINI_LOGO_H, MINI_LOGO_W, 
	MINI_LOGO_X, MINI_LOGO_Y, MINI_LOGO_ADD
	unsigned char mini_logo[MINI_LOGO_H*MINI_LOGO_W] 
*/
#include <linux/linux_logo_mini.h>
...
#ifdef CONFIG_MY_LOGO

#define MY_LOGO_LINE   (MY_LOGO_W/8)
...
static void fbcon_setup(int con, int init, int logo)
{
	...
	int cnt;
	int step;
#if defined(CONFIG_MY_LOGO) && (MY_LOGO_H > LOGO_H)
    	logo_lines = (MY_LOGO_H + fontheight(p) - 1) / fontheight(p);
#else
...
#ifdef CONFIG_MINI_LOGO
    if (depth >= 8){
    /* paint mini_logo on linux_logo.*/
	 for (i = 0; i < smp_num_cpus + 1 ; i++){
	      unsigned char pix;
	      x = MINI_LOGO_X + i * MINI_LOGO_ADD;
	      if( x < 0 || x + MINI_LOGO_W > MY_LOGO_W) break ;
	      dst = &linux_logo[MINI_LOGO_Y * logo_w + x];
	      for(y1 = 0; y1 < MINI_LOGO_H; y1 ++)
		   for(x1 = 0; x1 < MINI_LOGO_W; x1 ++){
			pix = mini_logo[y1 * MINI_LOGO_W + x1];
			if(mini_logo[0] != pix) dst[y1 * logo_w + x1] = pix;
		   }
	 }
    }
#endif
