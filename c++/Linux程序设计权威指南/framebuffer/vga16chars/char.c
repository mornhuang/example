...
void vga16_ascii_char(int x0, int y0, int color, unsigned char c)
{
	...
	for (xi = 0, x = x0; xi < 8; xi++, x++){
		...
		vga16_set_pixel(x, y, color);
	}
}


void vga16_ascii_string(int x, int y, int color, unsigned char *str, int len)
{
	int i;
	int x0 = x;
	for(i=0; i<len; i++){
		vga16_ascii_char(x0, y, color, str[i]);
		x0 += 8;
	}
}

void vga16_chinese_char(int x0, int y0,
...
void vga16_chinese_string(int x, int y,
	unsigned long color, unsigned char *text, int len)
{
	...
	for (i=0;i<len;i++){
		vga16_chinese_char(x, y, color, line[2*i], line[2*i+1]);
		x += 16;
	}
}
...
void vga16_mixed_string(int x, int y, unsigned long color,
	unsigned char *text, int length)
{
        ...
        for(i=0,j=0; i<length;){
                ...
                if(hz){
                        vga16_chinese_string(x + cur_pos, y, color, &ch[j], (i-j)/2);
                } else{
                        vga16_ascii_string(x + cur_pos, y, color, &ch[j], i-j);
                }
                cur_pos += (i-j)*8;
                j = i;
        }
